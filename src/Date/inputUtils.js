...
export default function useDateInput({ locale, value, validRange, inputMode, onChange, }) {
    ...
    const onChangeText = (date) => {
        ...
        if (Number.isNaN(day) || Number.isNaN(year) || Number.isNaN(month)) {
            setError(getTranslation(locale, 'notAccordingToDateFormat', () => 'notAccordingToDateFormat')(inputFormat));
            return;
        }
        const finalDate = inputMode === 'end'
        ...
        if (!isWithinValidRange(finalDate)) {
            let errors = validStart && validEnd
                ? [
                    `${getTranslation(locale, 'mustBeBetween', () => 'mustBeBetween')(formatter.format(validStart), formatter.format(validEnd))}`,
                ]
                : [
                    validStart
                        ? getTranslation(locale, 'mustBeHigherThan', () => 'mustBeHigherThan')(formatter.format(validStart))
                        : '',
                    validEnd
                        ? getTranslation(locale, 'mustBeLowerThan', () => 'mustBeLowerThan')(formatter.format(validEnd))
                        : '',
                ];
