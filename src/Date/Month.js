...
function Month(props) {
    const { index, mode, date, dates, startDate, endDate, onPressYear, selectingYear, onPressDate, scrollMode, primaryColor, selectColor, roundness, disableWeekDays, locale, validRange, } = props;
            ...
            return {
                ...
                generatedDays: days.map((_, dayIndex) => {
                    ...
                    if (mode === 'range') {
                        ...
                        if ((isFirstDayOfMonth && selectedEndDay) ||
    ...
    return (React.createElement(View, { style: [styles.month, { height: getMonthHeight(scrollMode, index) }] },
        React.createElement(View, { style: [
            ...
            React.createElement(TouchableRipple, { disabled: !isHorizontal, onPress: isHorizontal ? () => onPressYear(year) : undefined, accessibilityRole: "button", accessibilityLabel: `${monthName} ${year}`, style: [
                    styles.yearButton,
                    {
                ...
                React.createElement(View, { style: [
                    ...
                    React.createElement(Text, { style: [styles.monthLabel, theme.fonts.medium], selectable: false },
                        monthName,
                        " ",
                        year),
                    React.createElement(View, { style: isHorizontal ? styles.opacity1 : styles.opacity0 },
                        React.createElement(IconButton, { onPress: isHorizontal ? () => onPressYear(year) : undefined, icon: selectingYear ? 'chevron-up' : 'chevron-down' }))))),
        grid.map(({ weekIndex, generatedDays }) => (React.createElement(View, { style: styles.week, key: weekIndex }, generatedDays
            .filter((gd) => showWeekDay(gd.dayIndex, disableWeekDays))
            .map((gd) => gd.beforeWeekDay || gd.afterWeekDay ? (React.createElement(EmptyDay, { key: gd.dayIndex })) : (React.createElement(Day, { key: gd.dayIndex, theme: theme, day: gd.dayOfMonth, month: gd.month, year: gd.year, selected: gd.selected, inRange: gd.inRange, leftCrop: gd.leftCrop, rightCrop: gd.rightCrop, onPressDate: onPressDate, isToday: gd.isToday, selectColor: selectColor, primaryColor: primaryColor, disabled: gd.disabled, textColorOnPrimary: textColorOnPrimary }))))))));
}
