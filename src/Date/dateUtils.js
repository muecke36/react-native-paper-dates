...
export function useRangeChecker(validRange) {
    ...
    const startUnix = validStart instanceof Date
    ...
    const isDisabled = React.useCallback((day) => {
        return validDisabledDatesRef.current
            ? validDisabledDatesRef.current.some((disabledDate) => areDatesOnSameDay(disabledDate, day))
            : false;
 ...
 */
export function isDateWithinOptionalRange(date, { startUnix, endUnix, }) {
...
export function useInputFormatter({ locale }) {
...
export function useInputFormat({ formatter, }) {
    return React.useMemo(() => {
