...
function Calendar(props) {
    const { locale, mode, onChange, startDate, endDate, date, disableWeekDays, dates, validRange, dateMode, } = props;
    ...
    return (React.createElement(View, { style: styles.root },
        React.createElement(Swiper, { initialIndex: getInitialIndex(firstDate), selectedYear: selectedYear, scrollMode: scrollMode, renderItem: ({ index }) => (React.createElement(Month, { locale: locale, mode: mode, key: index, validRange: validRange, index: index, startDate: startDate, endDate: endDate, date: date, dates: dates, onPressYear: onPressYear, selectingYear: selectingYear, onPressDate: onPressDate, scrollMode: scrollMode, primaryColor: theme.colors.primary, selectColor: selectColor, roundness: theme.roundness, disableWeekDays: disableWeekDays })), renderHeader: ({ onPrev, onNext }) => (React.createElement(CalendarHeader, { locale: locale, onPrev: onPrev, onNext: onNext, scrollMode: scrollMode, disableWeekDays: disableWeekDays })) }),
        scrollMode === 'horizontal' ? (React.createElement(YearPicker, { selectedYear: selectedYear, selectingYear: selectingYear, onPressYear: onPressYear })) : null));
}
