...
function DayNames({ disableWeekDays, locale, }) {
    ...
    return (React.createElement(View, { style: [styles.dayNames, { backgroundColor: theme.colors.surface }], pointerEvents: 'none' }, shortDayNames
        .filter((_, dayIndex) => showWeekDay(dayIndex, disableWeekDays))
        .map((dayName, i) => (React.createElement(DayName, { key: `${dayName}_${i}`, label: dayName })))));
}
