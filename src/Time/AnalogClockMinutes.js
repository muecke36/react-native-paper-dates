...
function AnalogClockMinutes({ minutes }) {
    const range = getMinuteNumbers(circleSize, 12);
    const color = useTextColorOnPrimary();
    return (React.createElement(React.Fragment, null, range.map((a, i) => {
        ...
        return (React.createElement(View, { key: i, pointerEvents: "none", style: [
            ...
            React.createElement(View, { style: styles.outerHourInner },
                React.createElement(Text, { style: isCurrent ? { color } : undefined, selectable: false }, isZero ? '00' : currentMinutes))));
    })));
}
